...
async def page(client):
    ...
    with ui.element('div').classes("w-full flex flex-wrap"):
        ...
        }, html_columns=[0]).on('cellClicked', on_result_click).style("box-sizing: border-box; flex: 1 0 50%; min-width: 600px")
        artist_grid = ui.aggrid({
            'columnDefs': [
                {'headerName': '', 'field': 'img', 'maxWidth':60, 'sortable': True},
                {'headerName': 'Artist', 'field': 'artist', 'sortable': True},
            ],
            'rowData': [],
            'rowSelection': 'multiple',
        }, html_columns=[0]).on('cellClicked', on_result_click).style("box-sizing: border-box; flex: 1 0 50%; min-width: 500px")
    ...
    select_grid = ui.aggrid({
        ...
        'rowData': [],
        'rowSelection': 'multiple',
    }, html_columns=[0]).on('cellClicked', on_sel_click)
